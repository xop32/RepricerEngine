...
@NotThreadSafe
public class KMDDBAccessor {

    private Connection conn = null;

    private boolean myConnection = true;

    public KMDDBAccessor(Connection conn) {
        this.conn = conn;
        this.myConnection = false;
    }

    public KMDDBAccessor() {
    }
        ...
        static {
            if (!initialized) {
                try {
                    ...
                    String host = AppConfig.getString("KMDDBHost");
                    String database = AppConfig.getString("KMDDBName");
                    int port = AppConfig.getInteger("KMDDBPort", 3306);
                    cpds.setJdbcUrl("jdbc:mysql://" + host + ":" + port + "/" + database
                            + "?useServerPrepStmts=false&rewriteBatchedStatements=true");
                    cpds.setUser(AppConfig.getString("KMDDBUsername"));
                    cpds.setPassword(AppConfig.getString("KMDDBPassword"));
